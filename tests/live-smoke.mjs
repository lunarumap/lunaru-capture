...
try{
  ...
  await page.goto(base+'?v=0.20.0');await page.locator('#newBtn:enabled').waitFor();
  assert.match(await page.textContent('header'),/TEST 0.20/);
  for(const file of ['index.html','abc.js','abc.css','capture-support.js']){
    const remote=await page.evaluate(async f=>{const r=await fetch(f+'?v=0.20.0',{cache:'reload'});if(!r.ok)throw Error('HTTP '+r.status);return await r.text();},file);
    assert.equal(remote,await fs.readFile(path.join(root,'abc',file),'utf8'),file+' must match tested source');
  }
  await page.click('#newBtn');await page.fill('#objectName','HTTPS smoke 0.20');
  ...
  console.log('PASS: HTTPS TEST 0.20 exact assets; real simulated-camera recording, reload and saved playback; no page errors. Physical phones not tested.');
